// backend/index.js (VERSIÓN RESTAURACIÓN FINAL v26.0)
...
bot.command('start', async (ctx) => {
    try {
        ...
        await ctx.replyWithMarkdownV2(escapeMarkdownV2(WELCOME_MESSAGE), Markup.inlineKeyboard([Markup.button.webApp('🚀 Abrir App', webAppUrl)]));
    } catch (error) { console.error('[Bot] Error en /start:', error); }
});
...
const server = app.listen(PORT, async () => {
    ...
    } catch (telegramError) { console.error("[SERVIDOR] ❌ ERROR AL CONFIGURAR TELEGRAM:", telegramError.message); }
});
