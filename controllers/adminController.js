// RUTA: backend/controllers/adminController.js (SOLUCIÓN EQUIPO v2)
...
const processWithdrawal = asyncHandler(async (req, res) => {
    ...
    // [SOLUCIÓN EQUIPO v2] - No poblamos aquí para evitar problemas en la sesión. Se obtiene el ID.
    ...
    // [SOLUCIÓN EQUIPO v2] - Obtenemos el ID del usuario directamente desde la transacción.
    ...
    try {
        ...
        if (status === 'completed') {
            ...
            // [SOLUCIÓN EQUIPO v2] - INICIO DE LA MODIFICACIÓN
            // En este punto, el retiro se considera definitivo. Incrementamos el contador totalWithdrawal del usuario.
            // Usamos $inc para una operación atómica segura.
            await User.findByIdAndUpdate(userId, { $inc: { totalWithdrawal: amount } }, { session });
            // [SOLUCIÓN EQUIPO v2] - FIN DE LA MODIFICACIÓN
...
const adjustUserBalance = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        // [SOLUCIÓN EQUIPO v2] - INICIO DE LA MODIFICACIÓN
        // Construimos la operación de actualización dinámicamente.
        if (type === 'admin_credit') {
            updateOperation = { $inc: { [`balance.${currencyKey}`]: amount } };
            // Si el crédito es en USDT, también incrementamos el contador de recarga total.
            if (currency === 'USDT') {
                updateOperation.$inc.totalRecharge = amount;
            }
        } else { // type === 'admin_debit'
            // Para el débito, primero verificamos el saldo para no dejarlo en negativo.
            const userCheck = await User.findById(id).select(`balance.${currencyKey}`).session(session);
            if (!userCheck || (userCheck.balance[currencyKey] || 0) < amount) {
                throw new Error('Saldo insuficiente para realizar el débito.');
            }
            updateOperation = { $inc: { [`balance.${currencyKey}`]: -amount } };
            // Nota: Un débito administrativo NO afecta el contador totalWithdrawal.
            // totalWithdrawal solo se afecta por retiros aprobados.
        }
        
        // Ejecutamos la actualización atómica y obtenemos el usuario actualizado.
...
async function _getBalancesForAddress(address, chain) { const TIMEOUT_MS = 15000; try { if (chain === 'BSC') { const bscProvider = new ethers.providers.JsonRpcProvider('https://bsc-dataseed.binance.org/'); const usdtBscContract = new ethers.Contract(USDT_BSC_ADDRESS, USDT_ABI, bscProvider); const [usdtBalanceRaw, bnbBalanceRaw] = await Promise.all([ promiseWithTimeout(usdtBscContract.balanceOf(address), TIMEOUT_MS), promiseWithTimeout(bscProvider.getBalance(address), TIMEOUT_MS) ]); const bnbFormatted = ethers.utils.formatEther(bnbBalanceRaw); return { usdt: parseFloat(ethers.utils.formatUnits(usdtBalanceRaw, 18)), bnb: parseFloat(bnbFormatted), trx: 0 }; } else if (chain === 'TRON') { const tempTronWeb = new TronWeb({ fullHost: 'https://api.trongrid.io', headers: { 'TRON-PRO-API-KEY': process.env.TRONGRID_API_KEY } }); tempTronWeb.setAddress(address); const usdtTronContract = await tempTronWeb.contract().at(USDT_TRON_ADDRESS); const [usdtBalanceRaw, trxBalanceRaw] = await Promise.all([ promiseWithTimeout(usdtTronContract.balanceOf(address).call(), TIMEOUT_MS), promiseWithTimeout(tempTronWeb.trx.getBalance(address), TIMEOUT_MS) ]); const trxFormatted = tempTronWeb.fromSun(trxBalanceRaw); return { usdt: parseFloat(ethers.utils.formatUnits(usdtBalanceRaw.toString(), 6)), trx: parseFloat(trxFormatted), bnb: 0 }; } } catch (error) { console.error(`Error al obtener saldo para ${address} en ${chain}:`, error); throw new Error(`Fallo al escanear ${address}. Causa: ${error.message}`); }}
const getTreasuryWalletsList = asyncHandler(async (req, res) => { const wallets = await CryptoWallet.find({}).select('address chain user').populate('user', 'username').lean(); const walletsWithDetails = await Promise.all(wallets.map(async (wallet) => { try { const balances = await _getBalancesForAddress(wallet.address, wallet.chain); let estimatedRequiredGas = 0; if (balances.usdt > 0.000001) { if (wallet.chain === 'BSC') { estimatedRequiredGas = await gasEstimatorService.estimateBscSweepCost(wallet.address, balances.usdt); } else if (wallet.chain === 'TRON') { estimatedRequiredGas = await gasEstimatorService.estimateTronSweepCost(wallet.address, balances.usdt); } } return { ...wallet, usdtBalance: balances.usdt, gasBalance: wallet.chain === 'BSC' ? balances.bnb : balances.trx, estimatedRequiredGas: estimatedRequiredGas }; } catch (error) { console.error(`Error al obtener detalles para la wallet ${wallet.address}: ${error.message}`); return { ...wallet, usdtBalance: 0, gasBalance: 0, estimatedRequiredGas: 0 }; } })); res.json(walletsWithDetails); });
...
const analyzeGasNeeds = asyncHandler(async (req, res) => { const { chain } = req.body; if (!['BSC', 'TRON'].includes(chain)) { res.status(400); throw new Error("Cadena no válida."); } let centralWalletBalance = 0; try { const { bscWallet, tronWallet } = transactionService.getCentralWallets(); const bscProvider = new ethers.providers.JsonRpcProvider('https://bsc-dataseed.binance.org/'); const tronWebInstance = new TronWeb({ fullHost: 'https://api.trongrid.io', headers: { 'TRON-PRO-API-KEY': process.env.TRONGRID_API_KEY } }); if (chain === 'BSC') { centralWalletBalance = parseFloat(ethers.utils.formatEther(await bscProvider.getBalance(bscWallet.address))); } else { centralWalletBalance = parseFloat(tronWebInstance.fromSun(await tronWebInstance.trx.getBalance(tronWallet.address))); } } catch (error) { console.error("CRITICAL ERROR: Fallo al obtener el balance de la billetera central (Dispensador):", error); centralWalletBalance = 0; } const walletsInChain = await CryptoWallet.find({ chain }).lean(); const walletsNeedingGas = []; for (const wallet of walletsInChain) { try { const balances = await _getBalancesForAddress(wallet.address, chain); const gasBalance = chain === 'BSC' ? balances.bnb : balances.trx; if (balances.usdt > 0.000001) { let requiredGas = 0; if (chain === 'BSC') { requiredGas = await gasEstimatorService.estimateBscSweepCost(wallet.address, balances.usdt); } else { requiredGas = await gasEstimatorService.estimateTronSweepCost(wallet.address, balances.usdt); } const gasMissing = requiredGas - gasBalance; if (gasBalance < requiredGas - GAS_SUFFICIENT_TOLERANCE || gasBalance === 0) { walletsNeedingGas.push({ address: wallet.address, usdtBalance: balances.usdt, gasBalance: gasBalance, requiredGas: requiredGas }); } else { console.log(`[analyzeGasNeeds DEBUG] Wallet ${wallet.address} con ${balances.usdt} USDT. Gas: ${gasBalance.toFixed(8)}, Requerido: ${requiredGas.toFixed(8)}. Gas Faltante: ${gasMissing.toFixed(8)}. No necesita gas (ya tiene suficiente).`); } } } catch (error) { console.error(`No se pudo analizar la wallet ${wallet.address}: ${error.message}`); } } res.json({ centralWalletBalance, walletsNeedingGas }); });
const dispatchGas = asyncHandler(async (req, res) => { const { chain, targets } = req.body; if (!chain || !Array.isArray(targets) || targets.length === 0) { res.status(400); throw new Error("Petición inválida."); } const report = { summary: { success: 0, failed: 0, totalDispatched: 0 }, details: [] }; const sendFunction = chain === 'BSC' ? transactionService.sendBscGas : transactionService.sendTronTrx; for (const target of targets) { try { const txHash = await sendFunction(target.address, target.amount); report.summary.success++; report.summary.totalDispatched += parseFloat(target.amount); report.details.push({ address: target.address, status: 'SUCCESS', txHash, amount: target.amount }); } catch (error) { report.summary.failed++; report.details.push({ address: target.address, status: 'FAILED', reason: error.message, amount: target.amount }); } } res.json(report); });
